...
init_testing_todo_app(){
	...
	token_linda=$(curl -s -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' --data '{"username":"{linda}","password":"{password}","rememberMe":false}' http://localhost:8080/login | jq -r '.id_token')
	echo '${PURPLE}token for linda is: ${NO_COLOR}'
	echo token_tom
	upload_test_data
}
...
upload_test_data(){
	...
	do
   		curl -X -H "Authorization: Bearer ${token_linda}" POST http://localhost:8080/addTodo?todo-title=valami$nameIndex
	done
	echo "finished adding test todos. list of todos are:"
	curl -X -H "Authorization: Bearer ${token_linda}" POST  http://localhost:8080/list | json_pp
}
...
test_toggle_completed_by_id(){
	...
	curl -X -H "Authorization: Bearer ${token_linda}" PUT http://localhost:8080/todos/2/toggle_status?status=true
...
test_delete_completed(){
	...
	curl -X -H "Authorization: Bearer ${token_linda}" DELETE  http://localhost:8080/todos/completed 
...
test_toggle_all_to_completed(){
	...
	curl -X -H "Authorization: Bearer ${token_linda}" PUT http://localhost:8080/todos/toggle_all?toggle-all=true
...
test_delete_todo_by_id(){
	...
	curl -X -H "Authorization: Bearer ${token_linda}" DELETE http://localhost:8080/todos/3
...
test_update_title_by_id(){
	...
	curl -X -H "Authorization: Bearer ${token_linda}" PUT http://localhost:8080/todos/4?todo-title=newtitlefortest
...
test_get_title_by_id(){
	...
	curl -H "Authorization: Bearer ${token_linda}" http://localhost:8080/todos/4
	echo ''
